...
export default function OtizmTesti() {
    ...
    const analyzeRisk = (answers) => {
        ...
        if (kritikHayirSayisi >= 2 || toplamHayirSayisi >= 8) {
            return {
                risk: '🚨 Yüksek Risk',
                mesaj: 'Yaptığınız test sonucuna göre çocuğunuzun otizm açısından **yüksek risk taşıdığı** görülmektedir.**Önerilen Adımlar:**- En kısa sürede bir çocuk nöroloğu veya gelişim uzmanına danışmalısınız.- **Online Otizm Danışmanlığı** hizmetimizden yararlanarak gelişim sürecine destek olabilirsiniz. - Erken müdahale, çocuğunuzun gelişimi için büyük bir avantaj sağlar. '
            };
        } else if (kritikHayirSayisi === 1 || toplamHayirSayisi >= 3) {
            return {
                risk: '⚠️ Orta Risk',
                mesaj: 'Yaptığınız test sonucuna göre çocuğunuzda otizme işaret edebilecek bazı belirtiler olabilir. **Ne Yapmalısınız?**  -**Online Otizm Danışmanlığı** hizmetimizden yararlanarak gelişim sürecine destek olabilirsiniz.  - Bir çocuk gelişim uzmanına veya psikoloğa danışabilirsiniz.  '
            };
        } else {
            return {
                risk: '✅ Düşük Risk',
                mesaj: 'Yaptığız test sonucuna göre çocuğunuzun otizm riski düşük görünüyor. Ancak gelişimini takip etmeye devam edin. **Önerilen Adımlar:** - Çocuğunuzla bol bol göz teması kurarak konuşun ve oyunlar oynayın. - Eğer hala endişeleriniz varsa, gelişim takibi için ** Online Otizm Danışma** ile iletişime geçebilirsiniz.'
            };
        }
    };
